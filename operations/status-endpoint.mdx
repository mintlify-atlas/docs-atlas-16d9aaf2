---
title: "WAF Auto-Block Status Endpoint Reference"
sidebarTitle: "Status Endpoint"
description: "The GET /status endpoint exposes operational state: running flag, startup time, last poll time, and last cleanup time. No secrets are exposed."
---
...
<ResponseField name="lastCleanupAt" type="string | null (ISO 8601)">
  The UTC timestamp of the most recent TTL cleanup pass that processed at least one expired entry. Returns `null` until the first cleanup pass that modifies state runs after startup.
</ResponseField>
...
<Note>
  `lastSuccessfulPollAt` and `lastCleanupAt` are `null` until the first successful cycle completes after startup.
</Note>
