---
title: "WAF Auto-Block Logging Reference"
...
<AccordionGroup>
  ...
  <Accordion title="SQLite block store initialized at {DatabasePath}">
    **Level:** Information

    Emitted after the SQLite database file has been opened (or created) and the schema has been verified. `{DatabasePath}` is the resolved absolute path to the `.db` file. If this message does not appear, the storage layer failed to initialize.
  </Accordion>
